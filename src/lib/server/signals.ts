...
function getSubscriptionAmount(items: unknown): number {
	...
	return items.data.reduce((total, item) => {
		...
		const unitAmount =
			price && typeof price.unit_amount === 'number' && Number.isFinite(price.unit_amount)
				? price.unit_amount
				: 0;
...
async function resolveOwnerEmail(org: Organization): Promise<string | null> {
	...
	const notifications = isRecord(metadata.notifications) ? metadata.notifications : {};
	...
	try {
		...
		const email =
			payload.email_addresses?.find((entry) => entry.id === payload.primary_email_address_id)
				?.email_address ??
...
async function sendHighValueAlert(org: Organization, signal: ChurnSignal): Promise<void> {
	const ownerEmail = await resolveOwnerEmail(org);

	if (!ownerEmail) {
		log('warn', 'signals.high-mrr-alert', 'Skipping internal alert because owner email is unavailable', {
			org_id: org.id,
			signal_id: signal.id
		});
		return;
...
export async function detectHighMrrRisk(signal: ChurnSignal, org: Organization): Promise<void> {
	if (signal.mrr_amount <= 50_000) {
		return;
	}
...
export async function detectCardFailed(event: Stripe.Event, org: Organization): Promise<void> {
	...
	await insertSignal({
		...
		stripeCustomerId,
		customerEmail: invoice.customer_email ?? readString(metadata.customer_email),
