...
function createVariant(params: EmailTemplateParams): EmailVariant {
  ...
  const safeOrgName = sanitizeEmailHeaderValue(org_name);
  ...
  switch (`${signal.signal_type}:${step}`) {
    case "card_failed:1":
      return {
        ...
        paragraphs: [
          ...
          `Thanks for taking a minute to sort it out. ${safeOrgName}`,
        ],
    ...
    case "cancelled:1":
      return {
        ...
        paragraphs: [
          `We saw the cancellation come through and wanted to say thank you for giving ${safeOrgName} a real try.`,
    ...
    default:
      return {
        subject: `A quick note from ${safeOrgName}`,
        paragraphs: [
...
function renderHtml(variant: EmailVariant, orgName: string): string {
  const safeOrgName = sanitizeEmailHeaderValue(orgName);
          ...
          <table role="presentation" cellpadding="0" cellspacing="0" width="100%" style="max-width:600px;border-collapse:collapse;background-color:#111113;border:1px solid rgba(255,255,255,0.10);">
            <tr>
              <td style="padding:36px 32px 18px 32px;">
                ...
                <p style="margin:0 0 10px 0;font-family:Inter,Arial,sans-serif;font-size:13px;line-height:1.7;color:#8A8A92;">Sent by ChurnPulse on behalf of ${escapeHtml(safeOrgName)}</p>
...
function renderText(variant: EmailVariant, orgName: string): string {
  ...
  parts.push(`Sent by ChurnPulse on behalf of ${safeOrgName}`);
