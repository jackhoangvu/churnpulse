...
export function normalizeStripeEvent(
  ...
  const data = ((event.data as Record<string, unknown>) ?? {}) as Record<
    ...
    unknown
  >;
  const obj = ((data.object as Record<string, unknown>) ?? {}) as Record<
    string,
    unknown
  >;
  ...
  if (eventType === "customer.subscription.trial_will_end") {
    ...
    if (
      !customerId ||
      trialEnd >
        Date.now() + CHURN_THRESHOLDS.TRIAL_ENDING_DAYS * 24 * 60 * 60 * 1000
    ) {
      return null;
    }
...
export function normalizePaddleEvent(
  ...
  const data = ((event.data as Record<string, unknown>) ?? {}) as Record<
    ...
    unknown
  >;
  const customer = ((data.customer as Record<string, unknown>) ?? {}) as Record<
    string,
    unknown
  >;
  ...
  if (eventType === "transaction.payment_failed") {
    const totals = ((((data.details as Record<string, unknown>) ?? {})
      .totals as Record<string, unknown>) ?? {}) as Record<string, unknown>;
  ...
  if (eventType === "subscription.updated") {
    ...
    const previousData = ((data.previous_data as Record<string, unknown>) ??
      {}) as Record<string, unknown>;
    const previousItems = Array.isArray(previousData.items)
      ? previousData.items
      : [];
...
export function normalizeLemonSqueezyEvent(
  ...
  const data = ((event.data as Record<string, unknown>) ?? {}) as Record<
    ...
    unknown
  >;
  const attrs = ((data.attributes as Record<string, unknown>) ?? {}) as Record<
    string,
    unknown
  >;
  ...
  if (eventName === "subscription_updated") {
    ...
    const firstItem = ((attrs.first_subscription_item as Record<
      string,
      unknown
    >) ?? {}) as Record<string, unknown>;
...
export function normalizePolarEvent(
  ...
  const eventId = String(event.id ?? "");
  const customerId =
    extractString(obj.customer_id) ?? extractString(obj.customer);
  const customer = ((obj.customer as Record<string, unknown>) ?? {}) as Record<
    string,
    unknown
  >;
...
function extractSubscriptionMrr(items: unknown): number {
  ...
  const data = extractRecord(items).data;
  if (!Array.isArray(data)) {
    return 0;
  }

  return data.reduce((total, item) => {
    const normalizedItem = extractRecord(item);
    const quantity = extractNumber(normalizedItem.quantity) || 1;
    const price = extractRecord(normalizedItem.price);
...
function extractPaddleMrr(items: unknown[]): number {
  return items.reduce((total: number, item) => {
    const normalizedItem = extractRecord(item);
    const price = extractRecord(normalizedItem.price);
    const unitPrice = extractRecord(price.unit_price);
