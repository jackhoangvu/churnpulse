...
function resolveWeekDeltas(
  rows: ChurnSignal[],
  now: Date,
): DashboardStats["weekDeltas"] {
  ...
  return {
    ...
    recoveredMrr:
      sumRecovered(currentWeekRows) - sumRecovered(previousWeekRows),
    activeSignals:
      countActiveSignals(currentWeekRows) -
      countActiveSignals(previousWeekRows),
    recoveryRate:
      calculateRecoveryRate(currentWeekRows) -
      calculateRecoveryRate(previousWeekRows),
  };
}
...
export async function loadDashboardStats(
  ...
  const cacheKey = `${orgId}:${now.toISOString().slice(0, 16)}`;
  ...
  statsCache.set(cacheKey, {
    expiresAt: Date.now() + CACHE_TTL_MS.DASHBOARD_STATS,
    value: stats,
  });
