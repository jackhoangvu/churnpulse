...
const TONE_BY_SIGNAL: Record<SignalType, AIClassification["recommended_tone"]> =
  {
...
function getDefaultWinBackAngle(signalType: SignalType): string {
  return signalType === "card_failed"
...
export function getFallbackClassification(
  ...
  return {
    churn_reason:
      signal.ai_churn_reason ?? getDefaultChurnReason(signal.signal_type),
    win_back_angle:
      signal.ai_win_back_angle ?? getDefaultWinBackAngle(signal.signal_type),
...
function isRecommendedTone(
  value: unknown,
): value is AIClassification["recommended_tone"] {
  return (
    value === "empathetic" ||
    value === "urgent" ||
    value === "value_focused" ||
...
function normalizeClassification(
  ...
  const churnReason =
    typeof candidate.churn_reason === "string"
      ? candidate.churn_reason.trim()
      : "";
  const winBackAngle =
    typeof candidate.win_back_angle === "string"
      ? candidate.win_back_angle.trim()
      : "";
...
export async function classifyChurnSignal(
  signal: ChurnSignal,
): Promise<AIClassification> {
  const fallback = getFallbackClassification(signal);
