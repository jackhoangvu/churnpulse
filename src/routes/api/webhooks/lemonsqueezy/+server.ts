...
export const POST: RequestHandler = async ({ request }) => {
	...
	if (!verifyLemonSqueezySignature(payload, signature, LEMONSQUEEZY_WEBHOOK_SECRET)) {
	...
	if (!checkRateLimit(`ls-webhook:${eventId}`, 1, 60_000)) {
		return json({ received: true });
	}
	...
	void (async () => {
		try {
			const customData =
				((meta.custom_data as Record<string, unknown>) ?? {}) as Record<string, unknown>;
