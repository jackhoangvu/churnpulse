...
async function resolveOrgByPaddleWebhookSecret(
  secret: string,
...
export const POST: RequestHandler = async ({ request }) => {
  ...
  const eventId = String(
    event.notification_id ?? event.id ?? crypto.randomUUID(),
  );
  if (!(await checkRateLimit(`paddle-webhook:${eventId}`, 1, 60_000))) {
    return json({ received: true });
  }
  ...
  void (async () => {
    try {
      ...
      if (!organization) {
        organization = await resolveOrgByPaddleWebhookSecret(
