...
function parseSignalType(value: string | null): SignalType | null {
  return value && signalTypes.includes(value as SignalType)
    ? (value as SignalType)
    : null;
}

function parseSignalStatus(value: string | null): SignalStatus | null {
  return value && signalStatuses.includes(value as SignalStatus)
...
export const GET: RequestHandler = async ({ locals, url }) => {
  ...
  try {
    ...
    const since = resolveSince(
      parseDateRange(url.searchParams.get("date_range")),
    );
