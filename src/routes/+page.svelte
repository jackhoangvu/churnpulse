<script lang="ts">
  ...
  function buildStepIconPath(icon: StepItem["icon"]): string[] {
    ...
    if (icon === "bell") {
      return [
        "M12 20a2.5 2.5 0 0 0 2.3-1.5",
        "M6 9a6 6 0 1 1 12 0c0 4 2 5 2 5H4s2-1 2-5",
      ];
    }
...
<main id="landing" class="text-primary">
  ...
  <section class="hero" id="hero">
    ...
    <div class="hero__content" id="hero-content">
      ...
      <p class="hero__subtitle">
        ChurnPulse connects to Stripe, Paddle, Lemon Squeezy, or Polar in 60
        seconds and automatically detects 7 churn signals including card
        failures, disengagement, plan downgrades, and more, then fires
        AI-personalized win-back emails before customers cancel.
      </p>

      <div class="hero__ctas" id="hero-actions">
        <a class="btn btn-primary btn-lg" href="/sign-up" id="hero-cta-primary"
          >Start free →</a
        >
        <a class="btn btn-ghost btn-lg" href="/demo" id="hero-cta-demo"
          >See live demo</a
        >
      </div>
              ...
              <tbody class="table__body">
                {#each previewRows as previewRow, index (previewRow.customer)}
                  ...
                  >
                    <td class="text-primary">
                      ...
                      >
                        {#if previewRow.statusClass}
                          <span
                            class={`status-dot ${previewRow.statusClass}`}
                            aria-hidden="true"
                          ></span>
                        {/if}
                    ...
                    <td class="text-primary">
                      <span class={`badge ${previewRow.signalClass}`}
                        >{previewRow.signal}</span
                      >
                    </td>
                    <td class="font-mono text-primary">{previewRow.mrr}</td>
                    <td class="text-primary">
                      <span class={`badge ${previewRow.badgeClass}`}
                        >{previewRow.badgeLabel}</span
                      >
                    </td>
                  </tr>
                {/each}
  ...
  <section class="section platforms-section reveal" id="platforms">
    ...
    <div class="platforms-grid" id="platforms-grid">
      {#each [{ name: "Stripe", color: "#635BFF" }, { name: "Paddle", color: "#0EA5E9" }, { name: "Lemon Squeezy", color: "#FFC233" }, { name: "Polar", color: "#4F6EF7" }] as platform (platform.name)}
        <div
          class="platform-chip"
        ...
        >
          <span
            class="platform-chip__dot"
            aria-hidden="true"
            id={`platform-dot-${platform.name.toLowerCase().replace(" ", "-")}`}
          ></span>
          <span
            class="platform-chip__name"
            id={`platform-name-${platform.name.toLowerCase().replace(" ", "-")}`}
          >
            {platform.name}
          </span>
        </div>
  ...
  <section class="section reveal" id="signals">
    ...
    <p class="section__subtitle">
      Every signal maps to a specific failure mode inside a subscription
      business. ChurnPulse watches for all of them continuously so recovery
      starts before the cancellation email does.
    </p>
        ...
        >
          {#if signalCard.badgeLabel && signalCard.badgeClass}
            <div
              ...
              id={`signal-badge-${signalCard.title}`}
            >
              <span class={`badge ${signalCard.badgeClass}`}
                >{signalCard.badgeLabel}</span
              >
            </div>
          {/if}
  ...
  <section class="section reveal" id="how-it-works">
    ...
    <p class="section__subtitle">
      Connect once, let the system watch continuously, and intervene with
      signal-specific sequences while the account is still salvageable.
    </p>

    <div class="steps-grid" id="steps-grid">
      {#each stepItems as stepItem, index (stepItem.number)}
        <article
          class={`step reveal ${index === 0 ? "reveal-delay-1" : index === 1 ? "reveal-delay-2" : "reveal-delay-3"}`}
        >
  ...
  <section class="section reveal" id="pricing">
    ...
    <p class="section__subtitle">
      Choose the depth that matches your customer base today. Upgrade when churn
      becomes a board question instead of a founder instinct.
    </p>
    ...
    <p class="docs-callout">
      Beta users get lifetime 30% discount — limited spots remaining.
    </p>
  </section>
